...
#define HTTP_MAJOR_VERSION "HTTP/1."
...
static void wait_child(int sing);
static void become_daemon(void);
static void setup_environment(const char *root, char *user, char *group);
...
static void
setup_environment(const char *root, char *user, char *group)
{
	struct passwd *pw;
	struct group *gr;
...
static void
server_main(int server, const char *docroot)
{
	for (;;) {
...
become_daemon(void)
{
	...
	if (n != 0) { _exit(0); } // parent process finish
...
static struct HTTPRequest* read_request(FILE *in) {
	...
	if (req->length != 0) {
		if (req->length > MAX_REQUEST_BODY_LENGTH) {
			log_exit("request body too long (size: %d)", req->length);
		}

		req->body = xmalloc(req->length);
		if (fread(req->body, req->length, 1, in) < 1) {
			log_exit("failed to read request body: %s", req->body);
		}
	} else {
		req->body = NULL;
	}
...
static void read_request_line(struct HTTPRequest *req, FILE *in) {
	...
	*p++ = '\0';
	...
	if (strncasecmp(p, HTTP_MAJOR_VERSION, strlen(HTTP_MAJOR_VERSION)) != 0) {
		log_exit("parse error on request line (3): %s", buf);
	}
	p += strlen(HTTP_MAJOR_VERSION);   // p (3)
...
static long content_length(struct HTTPRequest *req) {
	...
	len = atol(val);
	if (len < 0) {
		log_exit("negative Content-Length value: %d", len);
	}
	...
	if (strcmp(req->method, "HEAD") != 0) {
		...
		for (;;) {
			...
			if (fwrite(buf, 1, n, out) < n) {
				log_exit("failed to write to socket: %s\n", strerror(errno));
			}
		}
...
static void signal_exit(int sig) {
	log_exit("exit by singnal %d", sig);
}
...
static void* xmalloc(size_t sz) {
	...
	p = malloc(sz);
	if (!p) {
		log_exit("failed to allocate memory (size: %d)", sz);
	}
